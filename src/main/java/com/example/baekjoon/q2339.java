...
public class q2339 {
  ...
  private static int[][] stones; //전역 변수 선언
  private static int solution = 0, n;
  ...
  private static int Solution() {
    //1: 불순물, 2: 보석 결정체
    int result_ver = GetSolution(0, n, 0, n, 1);
    int result_hor = GetSolution(0, n, 0, n, 2);

    if (result_ver + result_hor > 0) {
      if (IsStone(0, n, 0, n)) {
        return result_ver + result_hor;
      } else {
        return 1;
      }
    }
  ...
  public static int GetSolution(int x_start, int x_end, int y_start, int y_end, int mod) {
    ...
    for (int i = x_start; i < x_end; i++) {
      for (int j = y_start; j < y_end; j++) {
        if (stones[i][j] == 1 && IsJewel(i, j, x_start, x_end, y_start, y_end,
            mod)) {//mod=1 세로로 자른것,mod=2 가로로 자른것
          if (mod == 1) {
